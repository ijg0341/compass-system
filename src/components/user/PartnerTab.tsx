...
import { getPartners, getUser, createPartner, updateUser, checkUserIdDuplicate } from '@/src/lib/api/userApi';
...
export default function PartnerTab() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['partners', projectUuid, page, rowsPerPage],
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data: formData }: { id: number; data: PartnerFormData }) =>
  ...
  const handleOpenModal = async (partner?: ProjectUser) => {
    setApiError(null);
    setModalOpen(true);
    if (partner) {
      setIsLoadingDetail(true);
      try {
        const detail = await getUser(projectUuid, partner.id);
        setEditData(detail);
      } catch (err) {
  ...
  const handleSubmit = (formData: PartnerFormData) => {
    setApiError(null);
    if (editData) {
            ...
            <Table>
              ...
              <TableBody>
                {partners.length === 0 ? (
                  ...
                  partners.map((partner: ProjectUser) => (
                    <TableRow key={partner.id} hover>
                      ...
                      <TableCell>
                        <Typography
                          variant="body2"
                          sx={{
                            color: partner.is_active ? '#4CAF50' : '#F44336',
                            fontWeight: 600,
                          }}
                        >
                          {partner.is_active ? '로그인 가능' : '차단'}
                        </Typography>
                      </TableCell>
                      <TableCell>
                        <Button
