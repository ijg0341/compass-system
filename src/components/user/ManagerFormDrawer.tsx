...
export default function ManagerFormDrawer({
  ...
  useEffect(() => {
    if (editData) {
      // 백엔드는 role 필드 사용, 프론트엔드는 member_type으로 매핑
      const role = (editData as ProjectUser & { role?: ManagerMemberType }).role;
      setFormData({
  ...
  return (
    <>
      ...
      >
        <form id="manager-form" onSubmit={handleSubmit}>
          {error && (
          ...
          <TableContainer component={Paper} variant="outlined" sx={{ bgcolor: 'background.paper' }}>
            <Table size="small">
              <TableBody>
                ...
                {/* 회원구분 / 생년월일 */}
                <TableRow>
                  <TableCell sx={labelCellSx}>회원구분</TableCell>
                  <TableCell sx={valueCellSx}>
                  ...
                  <TableCell sx={labelCellSx}>생년월일</TableCell>
                  <TableCell sx={valueCellSx}>
                    <TextField
                      fullWidth
                      size="small"
