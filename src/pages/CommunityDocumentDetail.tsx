...
import { useDocument, useDeleteDocument } from '@/src/hooks/useBoard';
...
export default function CommunityDocumentDetailPage() {
  ...
  // API 훅
  const { data: post, isLoading, error } = useDocument(projectUuid, postId);
  ...
  const handleEdit = useCallback(() => {
    navigate(`/community/document/${postId}/edit`);
  }, [navigate, postId]);
        ...
        <Table size="small">
          <TableBody>
            ...
            <TableRow>
              ...
              <TableCell sx={valueCellSx}>{post.admin_user_name || '-'}</TableCell>
              ...
              <TableCell colSpan={3} sx={valueCellSx}>
                {post.board_files && post.board_files.length > 0 ? (
                  <Link
                    ...
                    sx={{ cursor: 'pointer' }}
                  >
                    {post.board_files[0].original_name} ({formatFileSize(post.board_files[0].file_size)})
                  </Link>
                ) : (
                  '-'
                )}
              ...
              <TableCell colSpan={3} sx={valueCellSx}>
                {post.board_files && post.board_files.length > 1 ? (
                  <Link
                    ...
                    sx={{ cursor: 'pointer' }}
                  >
                    {post.board_files[1].original_name} ({formatFileSize(post.board_files[1].file_size)})
                  </Link>
                ) : (
                  '-'
                )}
