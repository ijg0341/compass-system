...
import { useDocuments } from '@/src/hooks/useBoard';
...
export default function CommunityDocumentPage() {
  ...
  // API 훅
  const { data: postsData, isLoading, error, refetch } = useDocuments(projectUuid, params);
  ...
  const handleRowClick = useCallback((row: BoardPostListItem) => {
    navigate(`/community/document/${row.id}`);
  }, [navigate]);

  // 등록 페이지로 이동
  const handleCreate = useCallback(() => {
    navigate('/community/document/create');
  }, [navigate]);
